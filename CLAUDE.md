...
- **C&H fail-closed guard (2026-09-21):** `Quality\C&H\ch_pull.py` writes to
  `CH_RAW.tmp.xlsx`, requires at least 465 source rows and all eight standard
...
- **Blueline fail-closed guard (2026-09-22):** `Quality\Blueline\bl_pull.py`
  writes and validates `BL_RAW.tmp.xlsx`, requires at least 440 source rows
  and all eight standard enrichment columns, then atomically replaces
  `BL_RAW.xlsx`. A partial 33-row response was rejected; the 460-row healthy
  history was restored from the last published dashboard before publication.
  The same run restored C&H from 418 to its 488-row healthy baseline.
...
- **Skyline RAW fail-closed guard** — `Quality\Skyline\Skyline_pull.py` must write to `SKYLINE_RAW.tmp.xlsx`, validate the temp file is non-zero, enforce a 10,000-row source floor, then `os.replace()` it into `SKYLINE_RAW.xlsx`. Do not overwrite the existing RAW file when Google Sheets returns zero/partial data; this was the root of the July 23 drop below 50K rows.
- **VIP RAW fail-closed guard** — `Quality\VIP\vip_pull.py` must write to `VIP_RAW.tmp.xlsx`, validate the temp file is non-zero, enforce a 3,500-row source floor, then replace `VIP_RAW.xlsx`. On July 23/24, the upstream `VIP_Evaluations` tab itself had only 2,373 data rows vs the 4,149-row baseline, so the dashboard must keep the drop visible until the source sheet is restored.
...
- **Movement effective-date History repair** — `masterlist_appsscript_enhanced.gs` appends a missing History row for a processed movement's exact effective date when the employee was already removed from the daily active snapshot before History could create that row. `movement_reconcile.py` also self-heals the local `history_cache.csv` for processed movements missing that exact-date row before validation/dashboard patching; historical movements before the first cached History date are skipped to avoid false alarms.
- **Existing same-day attrition correction (2026-09-23)** — When History already contains an Active row on an attrition's effective date, `movement_reconcile.py` now corrects that row to Inactive, records the attrition date/change type, and rebuilds the dashboard. This fixed Enmenso, Rey Noel's September 23 effective-date snapshot after the notification had already been sent.
