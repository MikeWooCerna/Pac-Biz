...
- **Movement effective-date History repair** — `masterlist_appsscript_enhanced.gs` appends a missing History row for a processed movement's exact effective date when the employee was already removed from the daily active snapshot before History could create that row. `movement_reconcile.py` now validates this exact-date History coverage for processed movements; historical movements before the first cached History date are skipped to avoid false alarms.
